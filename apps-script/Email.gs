/**
 * Email.gs — Monthly email trigger and HTML template.
 */

/**
 * Send monthly summary email to all active members.
 * Calculates balances for the current month and emails each member.
 */
function sendMonthlyEmail() {
  ...
  // Don't send if no expenses this month
  ...
  for (var i = 0; i < members.length; i++) {
    ...
    var html = buildEmailHtml(member.name, month, groupName, currency, balances, settlements, expenses, sheetUrl);
    var subject = groupName + ' — ' + getMonthName(month) + ' Expense Summary';
 ...
 */
function buildEmailHtml(memberName, month, groupName, currency, balances, settlements, expenses, sheetUrl) {
  var totalSpent = 0;
...
/**
 * Create the monthly email trigger.
...
function createMonthlyTrigger() {
  ...
  for (var i = 0; i < triggers.length; i++) {
    if (triggers[i].getHandlerFunction() === 'sendMonthlyEmail') {
      ScriptApp.deleteTrigger(triggers[i]);
    }
  }

  // Create new monthly trigger on the 28th at 9 AM
